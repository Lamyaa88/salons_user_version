...
class GetAddressLocationScreen extends StatefulWidget {
  final double lat ;
  final double long ;

  GetAddressLocationScreen(this.lat , this.long);
  @override
  _GetAddressLocationScreenState createState() => _GetAddressLocationScreenState(lat ,long);
}
class _GetAddressLocationScreenState extends State<GetAddressLocationScreen> {
  final double lat ;
  final double long ;

  double addressLat ;
  double addressLont ;
  _GetAddressLocationScreenState(this.lat, this.long);
    ...
    setState(() {
      ...
      showCustomAlertDialog(context: context ,onTapClose: (){Navigator.of(context).pop();} , onTapOk: (){
        ...
        customPushNamedNavigation(context,
            AddAddressScreen(lat: addressLat , long:  addressLont , location: userPickedLocation,));

      } , alertTitle:
      translator.translate("Use This Location") , alertSubtitle: "${userPickedLocation} "
          "\n \n\n lat : ${addressLat.toString()} \n long ${addressLont.toString()}");
    });
  }
//
  @override
          ...
          child: GoogleMap(
            onTap:(latLng) async{
              ...
              setState(() {
                ...
                addressLont = latLng.longitude ;
                print("addressLat is ${addressLat} addressLont is ${addressLont}");
              });
